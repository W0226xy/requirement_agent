...
from requirement_agent.api.conversations import router as conversations_router
from requirement_agent.api.errors import application_error_handler
from requirement_agent.api.feishu import router as feishu_router
...
def create_app(settings: Settings | None = None) -> FastAPI:
    #settings：可选的配置对象。
    #可以传入一个 Settings 实例来覆盖默认配置。
    #如果没有传入，则会使用 get_settings() 函数获取默认配置。
