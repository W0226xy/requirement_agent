...
async def compact_conversation(
    ...
    async with session_factory() as session:
        ...
        facts = [{
            ...
            "需求提取": latest.get((item.source_record_id, AnalysisType.EXTRACTION)),
            "冲突分析": latest.get((item.source_record_id, AnalysisType.CONFLICT_RISK)),
            "审核状态": review_by_source.get(item.source_record_id),
        } for item in messages]
        summary_input = json.dumps(
            {"旧摘要": conversation.summary, "待压缩事实": facts}, ensure_ascii=False
        )
        new_summary = (
